...
static Gfx::Buffer s_gpuSceneConstantBuffers;
static Gfx::Buffer s_gpuPointLightBuffers;
static Gfx::Buffer s_gpuSpotLightBuffers;
DescriptorSet sceneDescriptorSets;
DescriptorSet textureDescriptorSets;
...
static bool InitGBufferPassData()
{
    ...
    pipelineDesc.descriptorSetLayouts   = s_descriptorSetLayouts; //gBufferPassData.descriptorSetLayout
...
bool InitPostProcessPassData()
{
    ...
    postProcessPassData.renderPass = g_renderState.device.NewRenderPass( desc, "postProcess" );
    if ( !postProcessPassData.renderPass )
    {
        return false;
    }
    ...
    info.sampler = "nearest_clamped_nearest";
    info.usage   = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT;
    postProcessPassData.colorAttachment = g_renderState.device.NewTexture( info, false, "offscreen color attachment" );

    VkImageView attachments[2];
    attachments[0] = postProcessPassData.colorAttachment.GetView();
    attachments[1] = g_renderState.depthTex.GetView();

    VkFramebufferCreateInfo framebufferInfo = {};
    framebufferInfo.sType           = VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO;
    framebufferInfo.renderPass      = postProcessPassData.renderPass.GetHandle();
    framebufferInfo.attachmentCount = 2;
    framebufferInfo.pAttachments    = attachments;
    framebufferInfo.width           = postProcessPassData.colorAttachment.GetWidth();
    framebufferInfo.height          = postProcessPassData.colorAttachment.GetHeight();
    framebufferInfo.layers          = 1;

    if ( vkCreateFramebuffer( g_renderState.device.GetHandle(), &framebufferInfo, nullptr, &postProcessPassData.frameBuffer ) != VK_SUCCESS )
    {
        LOG_ERR( "Could not create offscreen framebuffer" );
        return false;
    }
    PG_DEBUG_MARKER_SET_FRAMEBUFFER_NAME( postProcessPassData.frameBuffer, "offscreen pass (gbuffer)" );


    // Post Processing shaders
    auto fullScreenQuadVert = ResourceManager::Get< Shader >( "fullScreenQuadVert" );
    auto postProcessingFrag = ResourceManager::Get< Shader >( "postProcessFrag" );

    std::vector< DescriptorSetLayoutData > descriptorSetData = fullScreenQuadVert->reflectInfo.descriptorSetLayouts;
    ...
    postProcessPassData.descriptorSet = s_descriptorPool.NewDescriptorSets( 1, postProcessPassData.descriptorSetLayouts[0], "post process" )[0];
    ...
    postProcessingPipelineDesc.shaders[0]	          = fullScreenQuadVert.get();
    ...
    imageInfo.sampler = postProcessPassData.colorAttachment.GetSampler()->GetHandle();
    imageInfo.imageView = postProcessPassData.colorAttachment.GetView();

    VkWriteDescriptorSet descriptorWrite[1] = {};
    descriptorWrite[0].sType            = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    descriptorWrite[0].dstSet           = postProcessPassData.descriptorSet.GetHandle();
...
namespace RenderSystem
{
    ...
    bool Init()
    {
        ...
        s_descriptorPool = g_renderState.device.NewDescriptorPool( 3, poolSize, 3, "render system" );
        ...
        sceneDescriptorSets    = s_descriptorPool.NewDescriptorSet( s_descriptorSetLayouts[0], "scene data" );
        textureDescriptorSets  = s_descriptorPool.NewDescriptorSet( s_descriptorSetLayouts[1], "textures" );
        ...
        descriptorWrite[0].sType            = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
        descriptorWrite[0].dstSet           = sceneDescriptorSets.GetHandle();
        descriptorWrite[0].dstBinding       = 0;
        descriptorWrite[0].dstArrayElement  = 0;
        ...
        descriptorWrite[1].dstSet           = textureDescriptorSets.GetHandle();
        ...
        descriptorWrite[2].dstSet           = sceneDescriptorSets.GetHandle();
        ...
        descriptorWrite[3].dstSet           = sceneDescriptorSets.GetHandle();
        ...
        PipelineDescriptor pipelineDesc;
        pipelineDesc.renderPass             = &postProcessPassData.renderPass;
        pipelineDesc.descriptorSetLayouts   = s_descriptorSetLayouts;
        pipelineDesc.vertexDescriptor       = VertexInputDescriptor::Create( 4, bindingDesc, 4, attribDescs );
        pipelineDesc.rasterizerInfo.winding = WindingOrder::COUNTER_CLOCKWISE;
        pipelineDesc.viewport               = FullScreenViewport();
    ...
    void Shutdown()
    {
        ...
        postProcessPassData.colorAttachment.Free();
        postProcessPassData.renderPass.Free();
        vkDestroyFramebuffer( g_renderState.device.GetHandle(), postProcessPassData.frameBuffer, nullptr );
        ...
        for ( auto& layout : gBufferPassData.descriptorSetLayout )
        {
            layout.Free();
        }
    }
    ...
    void Render( Scene* scene )
    {
        ...
        cmdBuf.BindDescriptorSets( 1, &sceneDescriptorSets, gBufferPassData.pipeline, PG_SCENE_CONSTANT_BUFFER_SET );
        cmdBuf.BindDescriptorSets( 1, &textureDescriptorSets, gBufferPassData.pipeline, PG_2D_TEXTURES_SET );
        ...
        PG_DEBUG_MARKER_BEGIN_REGION( cmdBuf, "GBuffer Pass", glm::vec4( .8, .8, .2, 1 ) );
        cmdBuf.BeginRenderPass( postProcessPassData.renderPass, postProcessPassData.frameBuffer );
        PG_DEBUG_MARKER_BEGIN_REGION( cmdBuf, "GBuffer -- Rigid Models", glm::vec4( .2, .8, .2, 1 ) );
        cmdBuf.BindRenderPipeline( s_rigidModelPipeline );
        cmdBuf.BindDescriptorSets( 1, &sceneDescriptorSets, s_rigidModelPipeline, PG_SCENE_CONSTANT_BUFFER_SET );
        cmdBuf.BindDescriptorSets( 1, &textureDescriptorSets, s_rigidModelPipeline, PG_2D_TEXTURES_SET );
        ...
        */
        ...
        cmdBuf.BindDescriptorSets( 1, &postProcessPassData.descriptorSet, postProcessPassData.pipeline, 0 );
