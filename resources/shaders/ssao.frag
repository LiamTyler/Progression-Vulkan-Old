...
layout( set = 0, binding = 0 ) uniform sampler2D worldNormals;
...
void main()
{
    vec3 fragPos = ReconstructPosFromDepth( matrices.invP, UV, texture( depthBuffer, UV ).r );
    vec3 N       = ( matrices.V * vec4( DecodeOctVec( texture( worldNormals, UV ).xyz ), 0 ) ).xyz;
    
    ivec2 texDim       = textureSize( depthBuffer, 0 );
    ...
    for ( int i = 0; i < PG_SSAO_KERNEL_SIZE; ++i )
    {
        ...
        float offsetDepth = ReconstructPosFromDepth( matrices.invP, projCoords.xy, texture( depthBuffer, projCoords.xy ).r ).z;
